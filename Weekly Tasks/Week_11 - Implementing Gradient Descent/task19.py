...
class Value:
    ...
    def __add__(self, other):
        if type(other) == Value:
            ...
            return Value(self.data + other, [self, Value(other)], "+")
    ...
    def __pow__(self, other):
        if type(other) == Value:
            ...
            return Value(self.data ** other, [self, Value(other)], f"**{other}")
        
    def exp(self):
        return Value(np.exp(self.data), [self], "exp")
...
def main() -> None:
    ...
    x1w1_x2w2 = x1w1 + x2w2
    x1w1_x2w2.name = "x1w1 + x2w2"

    b = Value(6.8814, name="b")
    logit = x1w1_x2w2 + b
    logit.name = "logit"

    logit_2 = 2 * logit

    tanh = ((2 * logit).exp() - 1) / ((2 * logit).exp() + 1)

    tanh.backward()

    draw_dot(tanh).render(directory='./graphviz_output', view=True)

    """
    Here, the results differ a lot and I can't figure out why.
    """
