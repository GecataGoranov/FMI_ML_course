...
"""
    Here I'm doing backpropagation to calculate the derivatives.

    If the grad is np.nan and this is the first node from the first iteration,
    let the gradient be 1.

    If the operation, that led to this node is +, then just copy the gradients of the 
    child onto the parents (due to the fact that x = y + z => dx/dy = 1).

    If the operation, that led to this node is *, then multiply the child's gradient 
    by the other parent's value (due to the fact that x = y * z => dx/dy = z).

    If there are no parents, the algorithm stops.
"""
...
class Value:
    def __init__(self, data, prevs=None, op=None, grad=np.nan):
        self.data = data
        self._prev = prevs
        self._op = op
